...
ORDER BY s.subject";
	...
	$alphabet .= "<br /><label for=\"select_subject\">" . _("Subject:") . "</label> <select name=\"browser\" id=\"select_subject\" onChange=\"window.location=this.options[selectedIndex].value\" title=\"Databases by Subject\">
        $subs_option_boxes
        </select>";
...
function getDBbyTypeBoxes( $selected_type = "", $show_formats = true ) {
	...
	if ( $show_formats == true ) {
		$alphabet .= "<br /><label for=\"select_format\">" . _("Format:") . "</label> <select name=\"browser\" id=\"select_format\" onChange=\"window.location=this.options[selectedIndex].value\" title=\"Databases by Format\">
...
function listCollections( $search = "", $display = "default", $show_children = "false" ) {
	...
	switch ( $display ) {
      ...
      <div class=\"guide_list_bonus\">$list_bonus</ul></div>
			...
			$layout           .= "<div class=\"tab-pane guide_list active\" id=\"section-Collection\" role=\"tabpanel\" aria-labelledby=\"show-Collection\"><div class=\"guide_list_header\"><a name=\"section-Collection\"></a><h2>" . _( "Guides by metamajor" ) . "</h2></div><div class=\"row\">" . $col_1 . $col_2 . "</div></div>";
